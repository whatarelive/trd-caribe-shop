...
export const Modal: FC<Props> = ({ rootID, children, onDismiss }) => {
    ...
    return createPortal(
        <div 
            className="absolute top-0 bottom-0 left-0 right-0 bg-black/50 flex justify-center 
            items-center z-50"
        >
            <dialog 
                ref={dialogRef} 
                onClose={onDismiss}
                className="w-3/5 h-fit max-w-[500px] m-auto max-h-[500px] border-none rounded-md 
